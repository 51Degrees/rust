...
//! looks like. [`DidClient`](crate::DidClient) uses it, since it holds the
//! resource key and the licence key. [`redact`] is the backstop for code that
//! does not know them, and it works on shape alone, which is why
//! [`Error`](crate::Error) calls it on every free-text field it prints.
//!
//! The same rules are carried by the `redact` module of the
//! `fiftyone-pipeline-core` crate. The two are written out twice rather than
//! shared, because this crate is published on its own with a deliberately
//! small set of dependencies and takes none from the pipeline. Change the two
//! together.

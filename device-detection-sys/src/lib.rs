...
mod tests {
    ...
    const LITE_DATA_FILE: Option<&str> = option_env!("FIFTYONE_DD_LITE_DATA_FILE");
    ...
    fn smoke_detect_is_mobile() {
        ...
        unsafe {
            ...
            assert!(
                exception.is_okay(),
                "manager init should not raise an exception"
            );
            assert_eq!(
                status,
                StatusCode::Success,
                "manager init should succeed for the Lite data file"
            );

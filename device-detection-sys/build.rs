...
//! is exported as the `FIFTYONE_DD_LITE_DATA_FILE` compile time environment
...
fn main() {
    ...
    // On non MSVC platforms the Hash math routines pull in libm, and the atomic
    // helpers used by the resource manager pull in libatomic on some Linux
    // toolchains. The upstream CMakeLists.txt links both, so they are requested
    // here too. They are harmless when unused.
    ...
    if let Some(data_file) = find_lite_data_file(&cxx) {
        ...
        println!(
            "cargo:rustc-env=FIFTYONE_DD_LITE_DATA_FILE={}",
            data_file.display()
        );
    }
}
...
///
/// Prefers `FIFTYONE_DEVICE_DETECTION_CXX_DIR` when present, then falls back to
/// the sibling `../device-detection-cxx` directory next to the Rust workspace.
fn resolve_device_detection_cxx_dir() -> PathBuf {
    ...
    // two levels up from the crate then into `device-detection-cxx`.
    ...
    assert!(
        ...
        "could not find device-detection-cxx. Looked for {} \

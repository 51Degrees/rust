...
async fn home(
    ...
    Html(web_support::render_page(PageOptions {
        ...
        lead: "This example performs IP Intelligence on premise (from a local .ipi data file) \
               within a small axum web server. It shows the weighted network and location \
...
fn main() -> anyhow::Result<()> {
    // Resolve the on-premise data file through the three-tier scheme: it defaults
    // to a loadable tier (the 4.5 ASN file, or Enterprise when reachable) and
    // never the 4.4 Lite file, which the current native library rejects.
 ...
 *
 * Because the bundled data file is a free tier (the ASN file by default, never
 * the 4.4 Lite file which the current native library rejects), the page shows the
 * standard data-file warnings from `examples_shared::check_data_file`: a free-tier
 * notice and a stale-file notice when the file is more than 30 days old. These

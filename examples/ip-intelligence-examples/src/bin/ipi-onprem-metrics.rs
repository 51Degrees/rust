...
//! lookup took, whether the result came from a cache, and the weighting model
//! that IP Intelligence uses in place of Device Detection's match metrics. The
...
/// The property whose weighting distribution the metrics print summarises. For
/// the ASN data file this is the autonomous system; an Enterprise file would
/// also populate the location properties.
...
pub fn run(options: &ExampleOptions, out: &mut dyn Write) -> anyhow::Result<()> {
    ...
    writeln!(
        out,
        "Note: IP Intelligence uses a weighting model rather than Device \
         Detection's match metrics. Each candidate value carries a weighting \
         (0.0 to 1.0) indicating how strongly the data supports it; the values \
         above are ordered most probable first."
    )?;
...
/// weighting distribution of the summary property.
fn report_lookup(pipeline: &Arc<Pipeline>, ip: &str, out: &mut dyn Write) -> anyhow::Result<()> {
    ...
    let summary = ip_data.weighted_string(SUMMARY_PROPERTY);
    let (candidate_count, top) = match summary.value() {
        Ok(list) => (
            list.len(),
            list.first()
                .map(|w| format!("{} (weighting {:.4})", w.value, w.weighting())),
        ),
        Err(_) => (0, None),
    };
    ...
    writeln!(out, "\t{SUMMARY_PROPERTY} candidates: {candidate_count}")?;
    writeln!(
        out,
        "\tMost probable {SUMMARY_PROPERTY}: {}",
        top.as_deref().unwrap_or("(no value)")
    )?;
 ...
 * (difference, drift, method, iterations). Instead its results are
 * probabilistic: each property returns a list of candidate values, each with a
 * weighting in the range 0.0 to 1.0. This example therefore reports, per
 * lookup:
 *
 * 1. The wall-clock time the lookup took, measured around `FlowData::process`.
 * 2. Whether the result was served from a cache.
 * 3. The number of weighted candidates for a summary property and the most
 *    probable value with its weighting.
 *

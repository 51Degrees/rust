...
/// most-probable value of the compared property.
fn lookup(
    ...
    let value = ip_data.weighted_string(COMPARE_PROPERTY);
    Ok(match value.value() {
        Ok(list) => list
            .first()
            .map(|w| format!("{} (weighting {:.2})", w.value, w.weighting()))
 ...
 *
 * Each value is shown with its weighting, because IP Intelligence results are
 * probabilistic.
 *
 * # Data file
 *

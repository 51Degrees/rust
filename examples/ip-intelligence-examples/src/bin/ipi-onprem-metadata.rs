...
fn output_property_details(
    ...
    writeln!(out)?;
    writeln!(out, "Available properties ({}):", engine.properties().len())?;
    // Core metadata carries the value type and owning element data key. The
    // aspect metadata carries the description and the data tiers, so the two
    // are paired by property name.
    for property in engine.properties() {
        let aspect = engine
            .aspect_properties()
            .iter()
            .find(|a| a.name().eq_ignore_ascii_case(&property.name));
        ...
        writeln!(
            out,
            "\t{} [{:?}] in '{}' tier(s): {}",
            property.name, property.value_type, tiers, description
        )?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn metadata_lists_properties_and_evidence_keys() {
        ...
        // The headings and at least one typed property must appear.
        assert!(printed.contains("Available properties"));
        assert!(printed.contains("Accepted evidence keys"));
        assert!(printed.contains("CountryCode"));

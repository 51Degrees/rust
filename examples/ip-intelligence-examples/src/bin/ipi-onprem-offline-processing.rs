...
//! a local `.ipi` data file and writes the weighted results back as a YAML-like
...
/// Look one evidence record up and write its inputs and weighted results.
fn analyse_record(
    ...
    for property in REPORTED_PROPERTIES {
        let weighted = ip_data.weighted_string(property);
        let rendered = match weighted.value() {
            Ok(list) if !list.is_empty() => list
                .iter()
                .map(|w| format!("{}({:.2})", w.value, w.weighting()))
                .collect::<Vec<_>>()
                .join(", "),
            Ok(_) => "[]".to_owned(),
 ...
 * 3. Process each record and write the inputs and the weighted results back as
 *    a YAML-like document stream, suitable for piping to a file for later
 *    analysis.
 *

...
//!
//! # Three identifier kinds, global and licence scoped
//!
...
//!
//! Each *global* variant is unique across all callers; each *licence-scoped*
//! variant is unique only within the caller's own licence key.
//!
...
/// The property name for the licence-scoped probabilistic identifier, unique
/// only across the caller's own licence key.
...
/// The property name for the licence-scoped random identifier, a
/// server-generated GUID scoped to the caller's own licence key.
...
/// The property name for the licence-scoped hashed-email identifier, derived
/// from the supplied email and salt and scoped to the caller's licence key.
...
/// global and a licence-scoped variant.
...
pub trait FodIdData: AspectData {
    ...
    /// The licence-scoped 51Did as the raw base64 OWID envelope string. Property
    /// name [`ID_PROB_LIC_PROPERTY`].
    ///
    /// This value is unique only across the caller's own licence key, so it
    /// cannot be correlated with identifiers issued to other licence holders.
    ...
    /// The licence-scoped 51Did parsed into a [`FodId`]. Behaves like
    /// [`FodIdData::id_prob_global_fod_id`] for the licence-scoped value.
    ...
    /// The licence-scoped random 51Did as the raw base64 OWID envelope string.
    ...
    /// The licence-scoped random 51Did parsed into a [`FodId`].
    ...
    /// The licence-scoped hashed-email 51Did as the raw base64 OWID envelope
    ...
    /// The licence-scoped hashed-email 51Did parsed into a [`FodId`].
...
///
/// // Parsing an invalid envelope is a no-value, not a panic; the licence-scoped

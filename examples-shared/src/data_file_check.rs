...
pub fn check_data_file(engine: &dyn OnPremiseAspectEngine) -> Vec<String> {
    ...
    // The tier comes from the AspectEngine super-trait, so a Lite file is
    // flagged regardless of its age.
    if engine.data_source_tier().eq_ignore_ascii_case("lite") {
        warnings.push(
            "This example is using a 'Lite' data file. Lite data is for illustration only and \
             has limited accuracy and capabilities. The Enterprise file is described on the \

...
mod tests {
    ...
    /// Resolve the current 4.5 format ASN IP-intelligence data file at run time.
    ///
    /// The default test/CI data file is the ASN file (small, current format 4.5,
    /// loads reliably). The 4.4 Lite file is deliberately not used because it is
    /// rejected with `IncorrectVersion` against the current native source. Set
    /// `FIFTYONE_IPI_DATA_FILE` to override the path. Returns [`None`] when no
    /// data file is present, so the test skips cleanly on a machine without it.
    fn asn_data_file() -> Option<PathBuf> {
        if let Ok(path) = std::env::var("FIFTYONE_IPI_DATA_FILE") {

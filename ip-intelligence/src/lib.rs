...
impl IpIntelligencePipelineBuilder {
    ...
    ///     if let Ok(countries) = ip.country_code().value() {
    ///         if let Some(top) = countries.first() {
    ///             println!("country = {} (weighting {})", top.value, top.weighting());
    ///         }
    ///     }
    /// }
    /// # Ok(())
    ...
    ///
    /// if let Some(ip) = data.get(IP_DATA_KEY) {
    ///     if let Ok(asn) = ip.weighted_string("Asn").value() {
    ///         if let Some(top) = asn.first() {
    ///             println!("asn = {} (weighting {})", top.value, top.weighting());
    ///         }
    ///     }
    /// }
    /// # Ok(())

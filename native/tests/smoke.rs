...
///
/// Resolves the current 4.5 format ASN file (`51Degrees-IPIV4AsnIpiV41.ipi`),
/// which loads against this source revision. The older `51Degrees-LiteV41.ipi`
/// is the 4.4 format and is rejected with `IncorrectVersion`, so it is not used
/// here.
#[cfg(feature = "ipi")]
fn ipi_data_file() -> Option<PathBuf> {
    find_data_file(
        "FIFTYONE_IPI_DATA_FILE",
        "ip-intelligence-cxx",
...
///
/// Unlike the per-product smoke tests above, this test does NOT soft-skip. It
/// panics if either data file is missing or if either open or lookup errors, so
/// a reintroduced collision cannot pass silently. It prints the IsMobile value
/// and one IP Intelligence value so the proof is visible in the test output.
...
fn both_engines_coexist() {
    ...
    // The two real data files this proof loads. The Hash file is the current
    // Lite 4.1 format and the IP Intelligence file is the current 4.5 ASN
    // format. Both are required, so a missing file is a hard failure here.
    let dd_path = Path::new(
        "D:/Workspace/device-detection-cxx/device-detection-data/51Degrees-LiteV4.1.hash",
    );
    let ipi_path = Path::new(
        "D:/Workspace/ip-intelligence-cxx/ip-intelligence-data/51Degrees-IPIV4AsnIpiV41.ipi",
    );
    assert!(
        dd_path.is_file(),
        "the Device Detection Hash file must be present at {}",
        dd_path.display()
    );
    assert!(
        ipi_path.is_file(),
        "the IP Intelligence ASN file must be present at {}",
        ipi_path.display()
    );

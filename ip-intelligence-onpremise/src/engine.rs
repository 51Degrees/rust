...
mod tests {
    ...
    ///
    /// This resolves the current 4.5 format ASN file
    /// (`51Degrees-IPIV4AsnIpiV41.ipi`), which loads against this source
    /// revision and lets the real-lookup tests run an actual lookup. The older
    /// `51Degrees-LiteV41.ipi` is the 4.4 format and is rejected with
    /// `IncorrectVersion`, so it is not used. Set `FIFTYONE_IPI_DATA_FILE` to
    /// override the path.

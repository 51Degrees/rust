...
/// resolution. Resolution order: an explicit `FIFTYONE_DD_LITE_DATA_FILE`
...
fn lite_data_file() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("FIFTYONE_DD_LITE_DATA_FILE") {
...
#[test]
fn paid_tac_data_resolves_hardware_vendor() {
    let Some(file) = tac_data_file() else {
        eprintln!("no paid TAC Hash data file found; skipping paid-tier detection test");
        return;
    };

    let engine = match DeviceDetectionOnPremiseEngineBuilder::new(&file)
        .performance_profile(PerformanceProfile::HighPerformance)
        .build()
    {
        Ok(engine) => engine,
        Err(e) => {
            eprintln!("the paid TAC Hash data set did not load ({e}); skipping");
            return;
        }
    };
    ...
    // The paid Enterprise data resolves the hardware vendor, which the free Lite
    // file does not. For an iPhone User-Agent this is Apple.
    let vendor = device.hardware_vendor();
    assert!(
        vendor.has_value(),
        "the paid data file should resolve HardwareVendor (the Lite file does not)"
    );
    assert!(

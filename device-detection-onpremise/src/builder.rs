...
impl DeviceDetectionOnPremiseEngineBuilder {
    ...
    ///
    /// The native wrapper does not expose the tier embedded in the data set, so
    /// a caller that knows it (from the data file they shipped) can set it here
    /// to make missing-property reasoning precise. Defaults to `Lite`, the free
    /// tier.

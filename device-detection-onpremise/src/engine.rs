...
use fiftyone_device_detection_shared::{
    DeviceDataBase, UachJsConversionElement, DEVICE_DATA_KEY, DEVICE_ELEMENT_DATA_KEY,
    UACH_EVIDENCE_COOKIE_KEY, UACH_EVIDENCE_QUERY_KEY,
};
...
impl DeviceDetectionOnPremiseEngine {
    ...
    pub(crate) fn from_manager(
        ...
        let (properties, aspect_properties) = build_property_metadata(&manager, &data_source_tier);
        ...
        DeviceDetectionOnPremiseEngine {
            ...
            aspect_properties,
            evidence_key_filter,
            data_source_tier: data_source_tier
    ...
    fn detect(&self, data: &FlowData, device: &mut DeviceDataBase) -> Result<()> {
        ...
        for meta in &self.aspect_properties {
            ...
            match results.value_as_string(name, VALUE_SEPARATOR)? {
                Some(raw) => {
                    device.insert(name, native_value(&raw, meta.core().value_type));
                }
                None => {
...
/// The natural value type for a well-known device-detection property name, so
/// the declared metadata matches how the shared accessors coerce the value.
///
/// Anything not listed is treated as a string, which is the lossless default
/// for a value read from the native string reader.
fn known_property_type(name: &str) -> PropertyValueType {
    match name.to_ascii_lowercase().as_str() {
        "ismobile" | "iscrawler" | "issmartphone" | "istablet" | "isconsole" | "issmalldesktop"
        | "ismediahub" | "isemailbrowser" | "isereader" => PropertyValueType::Bool,
        "screenpixelswidth" | "screenpixelsheight" | "bitspersecond" | "releaseage" => {
            PropertyValueType::Integer
        }
        "screenmmwidth"
        | "screenmmheight"
        ...
        | "screenincheswidth"
        | "pricinglow"
        | "pricinghigh" => PropertyValueType::Double,
...
/// form. When the text does not parse as the declared type the value is kept as
/// a string so nothing is lost, and the typed accessor then reports a
/// wrong-type no-value rather than fabricating a number.
fn native_value(raw: &str, value_type: PropertyValueType) -> PropertyValue {
    match value_type {
        ...
        PropertyValueType::Integer => match raw.trim().parse::<i64>() {
            Ok(value) => PropertyValue::Integer(value),
            Err(_) => PropertyValue::String(raw.to_owned()),
        },
        PropertyValueType::Double => match raw.trim().parse::<f64>() {
            Ok(value) => PropertyValue::Double(value),
            Err(_) => PropertyValue::String(raw.to_owned()),
        },
        PropertyValueType::StringList => {
            ...
            let parts: Vec<String> = raw
                ...
                .map(str::to_owned)
                .collect();
            PropertyValue::StringList(parts)
        }
        _ => PropertyValue::String(raw.to_owned()),
    }
}
...
mod tests {
    ...
    fn native_bool_words_parse_both_cases() {
        ...
        // An unexpected boolean rendering is kept as a string rather than guessed.
        assert_eq!(
            native_value("maybe", PropertyValueType::Bool),
    ...
    fn native_numbers_parse_and_fall_back() {
        assert_eq!(
            ...
            PropertyValue::Integer(1170)
        );
        assert_eq!(
            native_value("71.5", PropertyValueType::Double),
            PropertyValue::Double(71.5)
        );
        // A non-numeric value for a numeric property is preserved as a string.
        assert_eq!(
            native_value("n/a", PropertyValueType::Integer),
            PropertyValue::String("n/a".to_owned())
        );
    }
    ...
    fn native_list_splits_on_separator() {
        ...
        assert_eq!(
            value,
            PropertyValue::StringList(vec!["iPhone".to_owned(), "iPhone 15".to_owned()])
        );
    }

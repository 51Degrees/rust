...
fn main() {
    ...
    // The atomic intrinsics used by the threading layer need libatomic on some
    // Linux toolchains, and the math library is required by the graph and
    // weighted value code. These mirror the link libraries in the upstream
    // CMakeLists.txt.
    if !build.get_compiler().is_like_msvc() {
...
fn resolve_ipi_cxx_dir() -> PathBuf {
    ...
    // two levels up from the crate then into `ip-intelligence-cxx`.
    ...
    assert!(
        ...
        "could not find ip-intelligence-cxx. Looked for {} (set FIFTYONE_IP_INTELLIGENCE_CXX_DIR to override)",
        sibling.display()
    );
    sibling
}

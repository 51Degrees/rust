...
mod tests {
    ...
    /// Path to the bundled Lite IP Intelligence data file, if present.
    fn lite_data_file() -> Option<PathBuf> {
        let path = ipi_cxx_dir()
            .join("ip-intelligence-data")
            .join("51Degrees-LiteV41.ipi");
        if path.is_file() {
            Some(path)
        } else {
            None
    ...
    fn lite_lookup_round_trip() {
        let Some(data_file) = lite_data_file() else {
            ...
            eprintln!("Lite .ipi data file not found, skipping lookup round trip");
            return;
        };
        ...
        unsafe {
            ...
            // 4.4, while this source revision targets 4.5 (the reduced data
            // file format). When the file is version skewed the engine
            // correctly reports `IncorrectVersion`, so that outcome is
            // accepted as the documented data-versus-source skew rather than a
            // binding failure. The full lookup below runs whenever the file
            // matches the source, which is the case once a 4.5 Lite file is in
            // place.
            assert!(
                status == StatusCode::Success || status == StatusCode::IncorrectVersion,
                "init should return Success or the version-skew status, got {status:?}: {}",
                exception_message(&mut exception)
            );

            if status != StatusCode::Success {
                eprintln!(
                    "Lite data file version skew ({status:?}); the source targets a newer \
                     format than the bundled file. Symbols linked and the init path executed."
                );
                // The manager was not populated, so nothing to free here.
                return;
            }
            ...
            // Read a network/location property as a string. Different Lite
            // builds expose different property sets, so accept any of a small
